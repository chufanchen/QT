...
class DecisionTransformer(TrajectoryModel):
    ...
    def forward(
        ...
        if self.config.embed_type == "gcn":
            ...
            edge_index = edge_index.permute(1, 0, 2).reshape(
                2, -1
            )  # 2 x (3 x seq_length-1)
            edge_index = edge_index.repeat(batch_size, 1, 1)
            offset = torch.arange(
                0, (seq_length - 1) * 3 * batch_size, (seq_length - 1) * 3
            ).view(-1, 1, 1)
            edge_index = edge_index + offset
            edge_index_state = edge_index.permute(1, 0, 2).reshape(
                2, -1
            )  # 2 x (3 x seq_length-1 x batch_size)

            edge_index = [
                torch.tensor([[0], [1]], dtype=torch.long) for j in range(1, seq_length)
            ]
            edge_index = torch.stack(edge_index, dim=0)
            ...
            edge_index = edge_index.permute(1, 0, 2).reshape(
                2, -1
            )  # 2 x (2 x seq_length-1)
            edge_index = edge_index.repeat(batch_size, 1, 1)
            offset = torch.arange(
                0, (seq_length - 1) * 2 * batch_size, (seq_length - 1) * 2
            ).view(-1, 1, 1)
            edge_index = edge_index + offset
            edge_index_action = edge_index.permute(1, 0, 2).reshape(
                2, -1
            )  # 2 x (2 x seq_length-1 x batch_size)

            # For state graph
            x = [
                torch.stack(
                    [
                        state_embeddings[j][i - 1],
                        action_embeddings[j][i],
                        state_embeddings[j][i],
                    ],
                    dim=0,
                )
            ...
            x = self.gcn_state(data.x, data.edge_index).view(
                batch_size, seq_length - 1, 3, -1
            )[:, :, 2]
            ...
            x = [
                torch.stack(
                    [
                        state_embeddings[j][i],
                        action_embeddings[j][i + 1],
                        returns_embeddings[j][i],
                    ],
                    dim=0,
                )
                for j in range(batch_size)
            ...
            x = torch.concat(x, dim=0)
            data = Data(x=x, edge_index=edge_index_state).to(device=device)
            x = self.gcn_return(data.x, data.edge_index).view(
                batch_size, seq_length - 1, 3, -1
            )[:, :, 2]
            ...
            x = [
                torch.stack(
                    [state_embeddings[j][i], action_embeddings[j][i + 1]], dim=0
                )
                for j in range(batch_size)
                for i in range(0, seq_length - 1)
            ]
            ...
            x = self.gcn_action(data.x, data.edge_index).view(
                batch_size, seq_length - 1, 2, -1
            )[:, :, 1]
